...
files = [
    r'c:\Users\Lenovo\OneDrive\Desktop\Travel Booking\index.html',
    r'c:\Users\Lenovo\OneDrive\Desktop\Travel Booking\future.html'
]
...
def process_file(file_path):
    ...
    # General fa- to iconify fallback (common icons)
    # This is a bit risky but we can target common ones
    content = re.sub(r'<i\s+class="fa-solid\s+fa-star"[^>]*></i>', '<iconify-icon icon="mdi:star"></iconify-icon>', content)
    content = re.sub(r'<i\s+class="fa-solid\s+fa-check"[^>]*></i>', '<iconify-icon icon="mdi:check"></iconify-icon>', content)
